...
const ProductList = () => {
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (value) {
      ...
      ;(window as any).debounce = setTimeout(() => {
        startTransition(async () => {
          const data = await adminGetAllProducts({ query: value, page: 1 })
          setData(data)
        })
      }, 500)
    } else {
      startTransition(async () => {
        const data = await adminGetAllProducts({ query: '', page })
        setData(data)
      })
    }
  }
  useEffect(() => {
    startTransition(async () => {
      const data = await adminGetAllProducts({ query: '' })
      setData(data)
    })
  }, [])
                ...
                <TableRow key={product._id}>
                  ...
                  <TableCell>
                    <Link href={`/admin/products/${product._id}`}>
                      {truncateProductName(product.name, 2)}
                    </Link>
                  </TableCell>
